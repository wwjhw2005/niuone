...
海外部署可通过 `DASHBOARD_CN_DATA_PROXY_URL=socks5h://host:port` 仅代理腾讯、东方财富、新浪和问财等国内数据源；该地址不得包含用户名、密码、查询参数或路径。配置代理后连接失败必须沿用有界超时、重试与缓存降级，不得静默改为直连。Docker Compose 会从容器路由表把回环代理主机映射为当前实际网关，宿主机防火墙须只对该 Compose 网段放行代理端口。模型、通知、FMP 和 NewsNow 不使用此配置。
